 ...
 *
 * A short is uploaded as privacyStatus "private" with status.publishAt set
 * to the target RFC3339 time; YouTube flips it public at that moment on its
 * own, so the schedule holds even if our runner never wakes again. A
 * long-form video is uploaded private with no publishAt and is left that
 * way: finalize() clears a go-live time instead of forcing it public.
 * Uploads use the resumable protocol (videos.insert, uploadType=resumable).
 * As a safety net for shorts, finalize() runs after the slot time and forces
 * the video public if YouTube did not flip it (some API projects have
 * publishAt silently ignored until they pass Google's audit).
 *
...
function buildBody(input: PublishInput): { body: YoutubeBody; scheduled: boolean; held: boolean } {
  const config = publisherConfig();
  const { item } = input;
  const publishAt = new Date(item.publishAt);
  const held = youtubeHoldsForReview(item);
  ...
  // A long-form upload never takes one: it stays private for review.
  const scheduled = !held && item.visibility === "public" && publishAt.getTime() > Date.now();
  const body: YoutubeBody = {
    ...
    status: {
      privacyStatus: held || scheduled ? "private" : item.visibility,
      ...(scheduled ? { publishAt: toRfc3339Utc(publishAt) } : {}),
      selfDeclaredMadeForKids: false
    }
  };
  return { body, scheduled, held };
}

async function writeYoutubeStatus(videoId: string, token: string, status: Record<string, unknown>): Promise<void> {
  try {
 ...
 * YouTube normally flips a short public itself via status.publishAt, but
 ...
 * A long-form video is the opposite: leave it private, and clear publishAt
 * so YouTube does not flip it.
 */
async function finalizeYoutube(item: QueueItem, state: PlatformState): Promise<PostResult> {
  ...
  if (youtubeHoldsForReview(item)) {
    if (status.privacyStatus === "public") {
      return { status: "published", postId: state.postId, detail: "already public on YouTube" };
    }
    if (status.publishAt || status.privacyStatus !== "private") {
      const nextStatus: Record<string, unknown> = { ...status, privacyStatus: "private" };
      delete nextStatus.publishAt;
      await writeYoutubeStatus(state.postId, token, nextStatus);
      return {
        status: "published",
        postId: state.postId,
        detail: "kept private for review - cleared the go-live time so YouTube will not publish it"
      };
    }
    return {
      status: "published",
      postId: state.postId,
      detail: "uploaded private for review - YouTube will not publish it"
    };
  }
  if (status.privacyStatus === "public") {
...
export const youtubeAdapter: PlatformAdapter = {
  ...
  buildPlan(input: PublishInput): PublishPlan {
    const config = publisherConfig();
    const { body, scheduled, held } = buildBody(input);
    ...
    return {
      ...
      notes: [
        held
          ? "Uploads as private and stays private. Make it public yourself after review."
          : scheduled
  ...
  async publish(input: PublishInput): Promise<PostResult> {
    ...
    const describe = (id: string): PostResult => ({
      ...
      postId: id,
      detail: held
        ? "uploaded private for review - YouTube will not publish it"
        : scheduled

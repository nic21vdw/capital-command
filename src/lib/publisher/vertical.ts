 ...
 * vertical format is what Reels and TikTok expect, so every post passes
 * through this gate before it is queued, hosted, or uploaded.
 */
 ...
 * when a YouTube post can't possibly be a Short (longer than three minutes) —
 * the boundary is strict, so even 180.0s files classify as long-form.
 */
export async function prepareVerticalMedia(absolutePath: string, platforms: PlatformId[]): Promise<PreparedMedia> {

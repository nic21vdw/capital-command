...
export async function runOverview(run: PipelineRun): Promise<PipelineRunOverview> {
  ...
  const readyItems =
    clipsReady + (longformReady ? 1 : 0) + (audioReady ? 1 : 0) + (slideCount > 0 ? 1 : 0) + posts + segmentsRendered;
  const upstreamSettled = (["longform", "segments", "clips", "audio", "images", "posts"] as const).every(

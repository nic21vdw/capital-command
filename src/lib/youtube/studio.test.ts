...
describe("deriveStudioOverview", () => {
  it("lists only YouTube items, newest-first by publish date", () => {
    ...
    expect(overview.rows[0].id).toBe("new");
    expect(overview.rows[1].id).toBe("old");
    expect(overview.publishedCount).toBe(2);
  });

  it("judges top and under performers against the channel median views", () => {
    const overview = deriveStudioOverview(
      [
  ...
  it("reaches no performance verdict on too few published videos", () => {
    const overview = deriveStudioOverview(
  ...
  it("names subscribers the bottleneck and projects watch hours from cadence", () => {
    const overview = deriveStudioOverview(
      [
  ...
  it("leaves an already-eligible channel with no bottleneck and no projection", () => {

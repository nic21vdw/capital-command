...
function captionsInRange(captions: CaptionSegment[], start: number, end: number) {
  return captions.filter((segment) => segment.end > start && segment.start < end);
}
...
function episodeCopy(captions: CaptionSegment[], sourceName: string, part?: number) {
  ...
  return {
    title,
    description: excerptWords ? `${excerptWords}${transcript.split(/\s+/).length > 72 ? "…" : ""}` : fallback,
 ...
 */
export function planChapterRanges(captions: CaptionSegment[], durationSec: number, sourceName: string) {
  ...
  while (startIndex < captions.length) {
    ...
    for (let index = startIndex; index < captions.length; index += 1) {
      ...
      const pauseAfter = next ? Math.max(0, next.start - captions[index].end) : Infinity;
      if (elapsed >= MAX_CHAPTER_SEC || (elapsed >= TARGET_CHAPTER_SEC && pauseAfter >= 1.4)) break;
    }
    ...
    if (remaining > 0 && remaining < MIN_CHAPTER_SEC) endIndex = captions.length - 1;
    const end = Math.min(durationSec, Math.max(start + 1, captions[endIndex].end));
  ...
  return ranges.filter((range, index) => range.endSec - range.startSec >= MIN_CHAPTER_SEC || index === 0);
}
...
export function planPodcastRanges(
  ...
  if (mode !== "full") ranges.push(...planChapterRanges(captions, duration, job.fileName));
  return ranges;
}

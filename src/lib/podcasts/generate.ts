...
import type { PodcastEpisode, PodcastGenerationMode } from "@/lib/podcasts/types";
import { publisherConfig } from "@/lib/publisher/config";
import { mediaHost } from "@/lib/publisher/hosting";

function stableEpisodeId(jobId: string, kind: string, start: number, end: number) {
...
async function sourceMediaPath(jobId: string) {
  ...
  if (job.sourceId) {
    ...
    if (!meta) throw new Error("The original uploaded livestream is no longer available.");
    return sourceFilePath(meta);
  }
  const cachedAudio = path.join(workDir(job.id), "source-audio.mp3");
  await access(cachedAudio).catch(() => {
    throw new Error("The cached livestream audio is missing. Re-run the livestream through Clip Generator first.");
  });
  return cachedAudio;
}

export async function generatePodcastEpisodes(jobId: string, mode: PodcastGenerationMode) {
  let job = await getJob(jobId);
  if (!job) throw new Error("That livestream job could not be found.");
  if (job.status !== "done") throw new Error("Wait for the livestream to finish processing before creating podcast audio.");
  if (!job.sourceCaptions?.length) job = (await fetchJobCaptions(jobId)) ?? job;

  const ranges = planPodcastRanges(job, mode);
  if (ranges.length === 0) throw new Error("No usable podcast ranges could be created from this livestream.");
  ...
  for (const range of ranges) {
    const id = stableEpisodeId(job.id, range.kind, range.startSec, range.endSec);
    ...
    const audioUrl = host
      ? await host.upload(output, `podcasts/audio/${fileName}`).then((key) => host.publicUrl(key))
      : undefined;
    episodes.push({
      id,
      jobId: job.id,

import { PODCAST_DESTINATIONS, type PodcastConnection } from "@/lib/podcasts/types";
...
export function podcastConnections(): PodcastConnection[] {
  ...
  const artworkReady = /^https:\/\//.test(process.env.PODCAST_ARTWORK_URL?.trim() || "");
  ...
  return PODCAST_DESTINATIONS.map((destination) => ({
    ...
    ready:
      destination.delivery === "rss"
        ? publicFeedReady && (destination.id !== "apple" || (artworkReady && ownerReady))
        : false,
    note:
      destination.delivery === "rss"
        ? publicFeedReady

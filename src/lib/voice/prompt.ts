...
export async function voiceInstructions(allowActions: boolean): Promise<string> {
  ...
  const tools = VOICE_TOOLS.filter((tool) => allowActions || !tool.action)
    ...
    .join("\n");
  return [
    `You are the voice of CoLateral Marketing, Nic's private content operations system. You are speaking to Nic out loud, so keep answers short and conversational — one or two sentences unless he asks for detail. No markdown, no bullet lists, no reading ids aloud unless he asks for one.`,
    OPERATOR,

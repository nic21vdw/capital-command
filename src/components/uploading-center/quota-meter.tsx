  ...
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2">
      <div className="flex items-center justify-between gap-4 text-xs">
        ...
        <span className={cn("tabular-nums", quota.overBudget ? "tone-warning tone-text" : "text-[var(--muted-foreground)]")}>
      ...
      <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
        <div
          className={cn("h-full rounded-full transition-[width]", quota.overBudget ? "tone-warning tone-fill" : "bg-[var(--accent)]")}
          style={{ width: `${percent}%` }}
        />
      </div>
      {quota.overBudget ? (
        <p className="tone-warning tone-text mt-1.5 flex items-center gap-1.5 text-[11px]">

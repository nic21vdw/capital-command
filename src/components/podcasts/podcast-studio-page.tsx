...
function EpisodeCard({
  ...
  const [scheduledAt, setScheduledAt] = useState(
    localDateTimeValue(episode.scheduledAt),
  );
  const [destinations, setDestinations] = useState<PodcastDestinationId[]>(
  ...
  const save = async (publishNow = false) => {
    ...
    try {
      ...
      toast.success(
        publishNow
          ? "Episode added to the live RSS feed."
          : scheduledAt
            ? "Episode scheduled."
            : "Draft saved.",
      );
  ...
  return (
    <Card className="space-y-4">
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <Badge>
                {episode.kind === "full" ? "Full episode" : "Topic episode"}
              </Badge>
      ...
      <audio
        ...
        controls
        preload="metadata"
        src={`/api/podcasts/audio?id=${encodeURIComponent(episode.id)}`}
      />
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 rounded-lg border px-2.5 py-2 text-left text-xs transition ${
                    selected
                      ? "border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_12%,transparent)] text-white"
                      : "border-[var(--border)] text-[var(--muted-foreground)] hover:text-white"
                  }`}
                >
                  <span
                    className={`flex h-4 w-4 items-center justify-center rounded border ${selected ? "border-[var(--accent)] bg-[var(--accent)] text-[var(--accent-contrast)]" : "border-[var(--border)]"}`}
                  >
...
export function PodcastStudioPage() {
  ...
  const selectedJob = useMemo(
    () => overview?.jobs.find((job) => job.id === selectedJobId),
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast.success(
        `Generated ${result.episodes?.length ?? 0} podcast audio file${result.episodes?.length === 1 ? "" : "s"}.`,
      );
      await load();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : String(error));
    ...
    <div>
      <PageHeader
        ...
        actions={
          <a
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-[var(--border)] px-4 py-2 text-sm text-white transition hover:bg-white/8"
          >
      ...
      <div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {overview.connections.map((connection) => (
          <Card key={connection.id} className="p-4">
            ...
            <p className="mt-1 min-h-10 text-xs leading-relaxed text-[var(--muted-foreground)]">
              {connection.note}
            </p>
            <a
      ...
      <Card className="mb-5">
          ...
          <div className="max-w-2xl">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-semibold text-white">
                Generate from a livestream
              </h2>
            </div>
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">
              The full MP3 is loudness-normalized for podcasts. Topic episodes
              follow transcript pauses and stay roughly 4–15 minutes long.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-[minmax(260px,1fr)_220px_auto]">
            <label className="text-xs font-medium text-[var(--muted-foreground)]">
              ...
              <select
                ...
                className="mt-1.5 w-full rounded-lg border border-[var(--border)] bg-[var(--panel)] px-3 py-2 text-sm text-white outline-none focus:border-[var(--accent)]"
              >
                {overview.jobs.length === 0 ? (
                  <option value="">No finished livestreams</option>
                ) : null}
            ...
            <label className="text-xs font-medium text-[var(--muted-foreground)]">
              ...
              <select
                value={mode}
                onChange={(event) =>
                  setMode(event.target.value as PodcastGenerationMode)
                }
                className="mt-1.5 w-full rounded-lg border border-[var(--border)] bg-[var(--panel)] px-3 py-2 text-sm text-white outline-none focus:border-[var(--accent)]"
              >
        ...
        {selectedJob ? (
          <p className="mt-3 text-xs text-[var(--muted-foreground)]">
            {durationLabel(selectedJob.durationSec)} source ·{" "}
            {selectedJob.hasTranscript
              ? "Transcript ready"
              : "Transcript will be generated"}{" "}
            · {selectedJob.episodeCount} existing outputs
          </p>
        ) : null}
      </Card>
          ...
          <Button
            ...
            onClick={() =>
              void navigator.clipboard
                .writeText(overview.feedUrl)
                .then(() => toast.success("RSS feed copied."))
            }
            className="gap-1.5"
          >
            <Rss className="h-3.5 w-3.5" /> Copy feed
          </Button>
        </div>
        {!overview.publicFeedReady ? (
          <p className="mt-2 text-xs text-amber-200">
            This local feed works for previewing, but podcast directories need a
            public HTTPS URL. Set PODCAST_PUBLIC_BASE_URL when Command is
            hosted.
          </p>
        ) : null}
      </Card>
      ...
      {overview.episodes.length === 0 ? (
        <Card className="border-dashed py-12 text-center">
          <Podcast className="mx-auto h-7 w-7 text-[var(--muted-foreground)]" />
          <p className="mt-3 text-sm font-medium text-white">
            No podcast audio yet
          </p>
          <p className="mt-1 text-xs text-[var(--muted-foreground)]">
            Choose a finished livestream above and generate the first package.
          </p>
        </Card>
      ) : (
        <div className="space-y-4">
          {overview.episodes.map((episode) => (
            <EpisodeCard
              key={`${episode.id}:${episode.updatedAt}`}
              episode={episode}
              onSaved={() => void load()}
            />
          ))}
